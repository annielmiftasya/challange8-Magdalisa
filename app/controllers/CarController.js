...
class CarController extends ApplicationController {
    ...
    try {
      ...
      const activeRent = await this.userCarModel.findOne({
        where: {
          carId:car.id,
          rentStartedAt: {
            [Op.gte]: new Date(rentStartedAt),
          },
          rentEndedAt: {
            [Op.lte]: new Date(rentEndedAt), 
          }
      ...
      if (!!activeRent) {
        const err = new CarAlreadyRentedError(car.name);
  ...
  handleUpdateCar = async (req, res,next) => {
    try {
      const car = await this.getCarFromRequest(req)
      const {
        name,
      ...
      const mobil = await this.carModel.update(
        {
            name,
            price,
            size,
